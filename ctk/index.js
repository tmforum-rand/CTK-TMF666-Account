...
var pmCollection = require('./CTK-Account-4.0.0.postman_collection.json');
...
Object.keys(config['payloads']).forEach(resource => {
    ...
    if (resource == "BillFormat") {
        pmCollection['item'][0]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
    }
    if (resource == "PartyAccount") {
        pmCollection['item'][3]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
    }
    if (resource == "BillingCycleSpecification") {
        pmCollection['item'][6]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
    }
    if (resource == "BillingCycleSpecification") {
        pmCollection['item'][6]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
    }
    if (resource == "FinancialAccount") {
        pmCollection['item'][9]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
    }
    if (resource == "BillingAccount") {
        pmCollection['item'][12]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
    }
    if (resource == "SettlementAccount") {
        pmCollection['item'][17]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
    }
});
